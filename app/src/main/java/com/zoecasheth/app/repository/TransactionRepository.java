...
public class TransactionRepository implements TransactionRepositoryType {
	...
	{
		return Single.fromCallable(() -> {
			Sign.SignatureData sigData = CryptoFunctions.sigFromByteArray(signatureBytes);
			if (sigData == null) return KeyService.FAILED_SIGNATURE.getBytes();
	...
	public Single<Transaction> storeRawTx(Wallet wallet, EthTransaction rawTx, long timeStamp)
	{
		...
		return Single.fromCallable(() -> {
			...
			Web3j web3j = TokenRepository.getWeb3jService(fetchedTx.getChainId().intValue());
