...
public class EthereumWriteBuffer extends DataOutputStream
{
    ...
    {
        write(com.zoecasheth.token.tools.Numeric.toBytesPadded(bi, 32));
    }

    public void writeAddress(BigInteger addr) throws IOException {
        write(com.zoecasheth.token.tools.Numeric.toBytesPadded(addr, 20));
    }

    public void writeAddress(String addr) throws IOException {
        BigInteger addrBI = new BigInteger(com.zoecasheth.token.tools.Numeric.cleanHexPrefix(addr), 16);
        writeAddress(addrBI);
    }

    public void writeBytes(String hex, int length) throws IOException {
        byte[] hexBytes = com.zoecasheth.token.tools.Numeric.hexStringToByteArray(hex);
    ...
    public void writeUnsigned4(BigInteger value) throws IOException {
        write(com.zoecasheth.token.tools.Numeric.toBytesPadded(UnsignedLong.create(value), 4));
    }
    ...
    public void writeTokenIds(List<BigInteger> tokenIds) throws IOException
    {
        ...
        {
            write(com.zoecasheth.token.tools.Numeric.toBytesPadded(tokenId, 32));
        }
    }
    ...
    public void write4ByteMicroEth(BigInteger weiValue) throws IOException
    {
        ...
        //convert to wei
        BigInteger microEth = com.zoecasheth.token.tools.Convert.fromWei(new BigDecimal(weiValue), Convert.Unit.SZABO).abs().toBigInteger();
