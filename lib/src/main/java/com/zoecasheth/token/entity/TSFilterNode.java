...
import static com.zoecasheth.token.entity.TSSelection.decodeParam;
...
public class TSFilterNode
{
    ...
    public com.zoecasheth.token.entity.FilterType type;
    boolean negate;
    public BigInteger value = null;
    String strValue = null;
    LogicState logic = LogicState.NONE;

    public TSFilterNode(String val, TSFilterNode p, com.zoecasheth.token.entity.FilterType t)
    {
        if (t == com.zoecasheth.token.entity.FilterType.ATTRIBUTE)
        {
    ...
    public TSFilterNode(com.zoecasheth.token.entity.FilterType type, TSFilterNode p)
    {
        this.type = type;
        parent = p;
    }
    ...
    public static boolean isLeafLogic(com.zoecasheth.token.entity.FilterType type)
    {
        switch (type)
        {
            case AND:
    ...
    LogicState evaluate(Map<String, com.zoecasheth.token.entity.TokenScriptResult.Attribute> attrs)
    {
    ...
    private BigInteger getBIValue(TSFilterNode node, Map<String, com.zoecasheth.token.entity.TokenScriptResult.Attribute> attrs)
    {
        BigInteger returnValue = null;
        if (node.strValue != null && node.strValue.length() > 0)
        {
            com.zoecasheth.token.entity.TokenScriptResult.Attribute attr = attrs.get(node.strValue);
            if (attr != null)
            {
                //found an attribute
    ...
    private String getValue(TSFilterNode node, Map<String, com.zoecasheth.token.entity.TokenScriptResult.Attribute> attrs)
    {
        ...
        else if (node.type == com.zoecasheth.token.entity.FilterType.ATTRIBUTE)
        {
